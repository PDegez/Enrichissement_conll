...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 14:58:01 2024
...
def get_argument_structure(sentence_nodes, id_verb):
    ...
    for id_argument in sentence_nodes.keys():
        if str(sentence_nodes[id_argument]["HEAD"]) == id_verb:
            ...
            # if sentence_nodes[id_argument]["DEPREL"] == "mod":
            #     syn += 1
            #     if sentence_nodes[id_argument]["UPOS"] == "ADP":
            #         prep = sentence_nodes[id_argument]["LEMMA"]
...
def main(input_file, output_file=None):
    ...
    annotation=0
    #data = cup.readConlluFile("../../conll/test_nent_lexhum.conll")
    data = cup.readConlluFile(input_file)
    sentences_json = []
    for index_sentence in range(len(data)) :
    # for index_sentence in range(10):
        # annotation conll :
        ...
        for id_token in sentence_nodes:
            if sentence_nodes[id_token]["UPOS"] == "VERB":
                ...
                if cles_dict:
                    ...
                    for arg in arg_hum :
                        data[index_sentence]["treeJson"]["nodesJson"][arg]["MISC"]["HUM_SCORE"] = int(data[index_sentence]["treeJson"]["nodesJson"][arg]["MISC"].get("HUM_SCORE", 0))+4
